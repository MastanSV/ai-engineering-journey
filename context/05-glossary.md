...
- **Cross-validation (k-fold)** — Split train into k folds; rotate which fold is held out; average score across k runs. Reduces dependence on a single 80/20 split.
- **Standardization** — Subtract mean, divide by std per feature. Mandatory before Ridge/Lasso so the L1/L2 penalty treats features fairly.
- **Regularization** — Add a penalty on weight magnitudes to the loss; trades a small amount of train fit for big gains in generalization.
- **L2 regularization (Ridge)** — Adds `λ·Σwⱼ²` to the loss; shrinks all weights toward zero, never exactly zero.
- **L1 regularization (Lasso)** — Adds `λ·Σ|wⱼ|` to the loss; can drive weights _exactly_ to zero → built-in feature selection.
- **alpha / λ** — Strength of the regularization penalty. α=0 → vanilla regression; α→∞ → all weights = 0.
- **Logistic regression** — Linear model + sigmoid → probability for binary classification. Loss = log-loss (cross-entropy), not MSE.
- **Sigmoid** — `σ(z) = 1/(1+e^-z)`. Squashes any real number into (0, 1) so it can be read as a probability.
- **Log-loss / cross-entropy** — `-[y·log(p) + (1-y)·log(1-p)]`. Convex in logistic regression's parameters (so GD finds the global min).
- **Decision boundary** — Surface in feature space where `p = 0.5` — the line/curve that separates the two predicted classes.
- **ROC / AUC** — ROC plots TPR vs FPR across thresholds; AUC = area under it. AUC = 0.5 random, 1.0 perfect.
